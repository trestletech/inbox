...
def test_is_mailing_list_message(set_up):
	result = set_up.is_mailing_list_message(USER_ID, NAMESPACE_ID, TEST_MSG['msg_id'])
	expected = True

	assert (result == expected)

def test_mailing_list_info_for_message(set_up):
	result = set_up.mailing_list_info_for_message(USER_ID, NAMESPACE_ID, TEST_MSG['msg_id'])
	expected = json.dumps(TEST_MSG['mailing_list_headers'], default=json_util.default)
	
	assert (result == expected)

def test_headers_for_message(set_up):
     result = set_up.headers_for_message(USER_ID, NAMESPACE_ID, TEST_MSG['msg_id'])
