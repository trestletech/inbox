...
def thread(message):
    msglist = db_session.query(Message).filter(Message.message_id.in_(message.references)).all()

    # Two cases for thread creation:
    # 1. First message in thread (by JWZ assumption)
    # 2. We haven't seen any of its earlier messages yet.
    if len(msglist) == 0:
        create_thread(message)

    # Else, add to thread.
    # Note: msglist could be incomplete!
    else:
        update_thread(msglist, message)

# TODO[kavya]: Children optimization
def update_thread(msglist, message):
    msgdict = { m.message_id: m for m in msglist }
    
    prev = None
    for msg_id in message.references:
        if msg_id in msgdict:
            ref = msgdict[msg_id]
            ...
            if prev is not None:
                ...
                if ref is message:
                    continue
                if ref.has_descendant(prev):
                    continue

                # Link already exists
                # TODO[kavya]: Check what if not prev?
                if ref.parent is not None:
                    continue

                else:
                    # TODO[kavya]: FIX - Prev need not be continuous ->
                    # CHECK - JWZ does this too?
                    prev.add_child(ref)
